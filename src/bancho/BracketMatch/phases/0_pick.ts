...
export const onMessage = (match: Match, msg: ChannelMessage) => {
  ...
  if (bansCommand) {
    const message = `Bans by ${match.teams[0].name}: ${match.teams[0].bans
      .map((x) => x.identifier)
      .join(', ')}`;
    const message2 = `Bans by ${match.teams[1].name}: ${match.teams[1].bans
      .map((x) => x.identifier)
      .join(', ')}`;
  ...
  if (pickList.available.includes(map)) {
    ...
    return payload
      ...
      .addMessage(
        `${team.name} chooses ${map.identifier} | [https://osu.ppy.sh/b/${beatmap_id} ${artist} - ${title} [${version}]] - [https://beatconnect.io/b/${beatmapset_id} Beatconnect Mirror] - [https://api.chimu.moe/v1/download/${beatmapset_id} chimu.moe Mirror]`,
      )
      .setState(1);
  }

  if (pickList.banned.includes(map))
    return payload.addMessage(`${map.identifier} was chosen as a ban`);

  if (pickList.tb.includes(map))
    return payload.addMessage(`Silly Goose! You can't pick the tiebreaker!`);

  if (pickList.doublePicks && pickList.doublePicks.includes(map))
...
export const onPhaseChange = (match: Match) => {
  ...
  return payload
    .addMessage(
      `${match.teams[0].name} | ${match.teams[0].score} - ${match.teams[1].score} | ${match.teams[1].name} // ${bestOfPhrase} // Next pick: ${team.name}`,
    )
