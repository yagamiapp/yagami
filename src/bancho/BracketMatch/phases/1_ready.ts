...
export const onReady = async (match: Match, lobby: BanchoLobby) => {
  ...
  const freemod = pick.mods.toUpperCase().includes('FREEMOD');
  if (freemod) {
    ...
    if (match.tournament.force_nf) {
      ...
      for (const player of lobby.slots) {
        ...
        const nf = player.mods.filter((x) => x.shortMod == 'nf').length == 0;
        if (nf) {
      ...
      if (noNF.length > 0) {
        ...
        return payload.addMessage(`The following players do not have the NF mod: ${noNFString}`);
      }
    }
