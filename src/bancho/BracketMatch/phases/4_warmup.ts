...
export const onPhaseChange = (match: Match, lobby: BanchoLobby) => {
  ...
  if (teamInLobby.length > 0) {
    ...
    payload.setHost(`#${newHost.id}`)
      .setMods('Freemod')
      .addMessage(`${newHost.username} has been selected to choose the warmup for ${team.name}. Use !skip to skip your warmup`)
...
const switchTeams = (match: Match) => {
  ...
  payload.setWaitingOn(nextWaitingOn)
    .setTeamWarmedUp(match.teams[match.waiting_on].id, true)
  ...
  if (team.warmed_up) {
    ...
    payload
      .clearHost()
      .setState(5)
      .applySettings(team_mode, score_mode, slots)
  }
  return payload;
}