...
export const onMessage = (match: Match, msg: ChannelMessage) => {
  ...
  const banList = getAvailableBans(match);
  if (banList.available.includes(map)) return payload
  ...
  if (banList.banned.includes(map)) return payload
    .addMessage(`${map.identifier} has already been chosen as a ban`)

  if (banList.tb.includes(map)) return payload
    .addMessage(`Silly Goose! You can't ban the tiebreaker!`)

  if (banList.doubleBans && banList.doubleBans.includes(map)) return payload
    .addMessage(`You cannot ban from the same modpool more than once`)
...
export const onPhaseChange = (match: Match) => {
  ...
  return payload
    ...
    .addMessage(`You have ${banOpportunities} ban${banOpportunities == 1 ? 's' : ''} left, use !list to see the available bans!`)
