...
export const onMessage = async (match: Match, msg: ChannelMessage) => {
  ...
  if (!command && msg.message.startsWith('!choose')) {
    ...
    return payload.addMessage(`Invalid command usage! Correct Usage: !choose [first|second] ${typeOption}`)
  }
  if (!command) return;

  const { order, type } = command.groups;
  if (match.round.bans === 0 && type == "ban") return payload.addMessage("There are no bans in this round, so you can't choose the ban order")
  if (type == 'pick' && team.pick_order) return payload.addMessage(`The pick order has already been chosen`)
  if (type == 'ban' && team.ban_order) return payload.addMessage(`The ban order has already been chosen`)
...
export const onPhaseChange = (match: Match) => {
  ...
  return new MatchPayloadBuilder()
    .addMessage(`${team.name}, it is your turn to pick! Use "!choose [first|second] ${typeOption}" to choose the order`)
