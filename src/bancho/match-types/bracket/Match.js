...
class MatchManager {
  ...
  async createMatch() {
    ...
    this.lobbySettings = {
      ...
      score_mode: this.tournament.score_mode == 4 ? 3 : this.tournament.score_mode,
    ...
    let invitesToIgnore = players.filter((player) => player).map((player) => player.user.username);
    ...
    this.lobby.on('playerJoined', async (event) => await this.joinHandler(event));
    this.lobby.on('allPlayersReady', async () => await this.readyHandler());
    this.lobby.on('matchFinished', async () => await this.finishHandler());
    this.lobby.on('beatmap', async (beatmap) => await this.beatmapHandler(beatmap));
  ...
  async joinHandler(event) {
    ...
    if (inLobbyPlayers.length > this.tournament.x_v_x_mode && this.state != 4) {
      return;
    }
    ...
    let teamSlots = [];
    for (let i = 1 + this.tournament.x_v_x_mode * teamIndex; i < this.tournament.x_v_x_mode; i++) {
      teamSlots.push(i);
    }

    if (slot && !teamSlots.includes(slot)) this.swaps.push({ player: event.player, slot });
  ...
  async readyHandler() {
    ...
    if (this.state == 1) {
      ...
      for (const team of this.teams) {
        ...
        for (const slot of this.lobby.slots) {
          if (team.players.includes(slot)) {
            lobbyCount[teamIndex] = lobbyCount[teamIndex] == null ? 1 : lobbyCount[teamIndex] + 1;
          }
        }
      }
      ...
      let freemod = this.picks[this.picks.length - 1].mods.toUpperCase().includes('FREEMOD');
      if (freemod) {
        ...
        if (this.tournament.force_nf) {
          ...
          for (const player of this.lobby.slots) {
            ...
            let nf = player.mods.filter((x) => x.shortMod == 'nf').length == 0;
            if (nf) {
          ...
          if (noNF.length > 0) {
            ...
            for (const user of noNF) {
              noNFString += noNFString == '' ? `${user}` : `, ${user}`;
            }
            await this.channel.sendMessage(
            ...
            if (userMap.includes(slot.user.username)) {
              ...
              if (modMap.some((x) => allowedFMMods.includes(x))) {
                freemodCount[teamIndex] =
                  freemodCount[teamIndex] == null ? 1 : freemodCount[teamIndex] + 1;
              }
            }
          }
    ...
    if (this.state == 2) {
      ...
      for (const team of this.teams) {
        ...
        if ([2, 4].includes(this.tournament.score_mode)) {
          score = ((score * 100) / this.tournament.x_v_x_mode).toFixed(3) + '%';
        }
        scoreString +=
          scoreString == ''
      ...
      for (const team of this.teams) {
        if (team.score >= scoreToWin) {
          await this.channel.sendMessage(`${team.name} has won the match! GGWP!`);
          await this.updateState(8);
          await this.channel.sendMessage(`The lobby will be closed in 90 seconds`);
      ...
      // Check for TB
      let tiebreakers = this.mappool.filter((x) => x.identifier.includes('TB'));
      ...
      if (tiebreakers.length > 0) {
        ...
        if (tb) {
          await this.channel.sendMessage(`It's a tie so far, time for the tiebreaker!`);
          let tb = this.mappool.filter((x) => x.identifier.toUpperCase().includes('TB'));
  ...
  async warmup() {
    ...
    for (const player of team.players) {
      ...
      if (slotMap.includes(player.user.username)) {
        ...
        await this.channel.sendMessage(
          `You have ${timers[this.state] / 60} minutes to start the warmup`
        );
        await this.startTimer(timers[this.state]);
        return;
      }
    }
    await this.channel.sendMessage(`Waiting for ${team.name} to join so they can get the host`);
  }
  ...
  async rollListener(msg) {
    ...
    if (this.rollVerification[username]) {
      ...
      if (team.roll == null) {
        ...
        await this.channel.sendMessage(`${team.name} rolled a ${roll.groups.roll}`);
  ...
  async chooseListener(msg) {
    ...
    if (user == null) return;
    let command = msg.message.match(/!choose (?<order>first|second) (?<type>pick|ban)/);
    ...
    if (this.round.bans == 0 && command.groups.type.toLowerCase() == 'ban') {
      await this.channel.sendMessage(
    ...
    if (command.groups.type.toLowerCase() == 'pick') {
      if (team.pick_order) {
        await this.channel.sendMessage(`The pick order has already been chosen`);
  ...
  async banListener(msg) {
    ...
    if (map.banned) {
      await this.channel.sendMessage(`${map.identifier} has already been chosen as a ban`);
      this.partials.banning = false;
      return;
    }
    ...
    if ((this.tournament.double_ban == 1 && map.mods != '') || this.tournament.double_ban == 0) {
      if (otherBanMods.includes(map.mods)) {
        await this.channel.sendMessage(`You cannot ban from the same modpool more than once.`);
        this.partials.banning = false;
        return;
      }
    }
    ...
    if (command.groups.map.substring(0, 2).toLowerCase() == 'tb') {
      await this.channel.sendMessage(`Silly goose! The tiebreaker is unbannable.`);
      this.partials.banning = false;
      return;
    }
    ...
    await this.channel.sendMessage(`${team.name} chooses to ban ${map.identifier}`);
    await this.updateMessage();
    await this.updateWaitingOn(1 - this.waiting_on);
    await this.banPhase();
    this.partials.banning = false;
  ...
  async pickListener(msg) {
    ...
    // Check if banned
    if (map.banned) {
      await this.channel.sendMessage(`${map.identifier} was one of the bans`);
      this.partials.picking = false;
      return;
    }
    ...
    if (map.picked) {
      await this.channel.sendMessage(`${map.identifier} has already been picked`);
      this.partials.picking = false;
      return;
    }
    ...
    if (
      ...
      if (lastTeamPick == map.mods) {
        await this.channel.sendMessage(`You cannot pick the same modpool twice.`);
        this.partials.picking = false;
        return;
      }
    }
    ...
    if (command.groups.map.substring(0, 2).toLowerCase() == 'tb') {
      await this.channel.sendMessage('Silly goose! The tiebreaker is unpickable.');
      this.partials.picking = false;
      return;
    }
  ...
  async listCommand(msg) {
    ...
    for (let map of this.mappool) {
      ...
      mapString += mapString.length > 0 ? `, ${map.identifier}` : map.identifier;
    }
  ...
  async bansCommand(msg) {
    ...
    for (const team of this.teams) {
      ...
      for (const ban of team.bans) {
        ...
        teamString +=
          teamString[teamString.length - 2] == ':' ? ban.identifier : `, ${ban.identifier}`;
      }
      if (teamString.length == team.name.length + 2) teamString = `${team.name}: None`;
  ...
  async msgHandler(msg) {
    ...
    if (msg.message == 'Countdown ends in 30 seconds' && msg.user.ircUsername == 'BanchoBot') {
    ...
    if (msg.message.match(/^!mp timer|^!mp aborttimer/g)) {
      ...
      if (team == this.teams[this.waiting_on] && timers[this.state]) {
        await this.channel.sendMessage('You have lost your priority for doing that.');
  ...
  async timerHandler(warn, excludeMessage) {
    ...
    await this.updateWaitingOn(1 - this.waiting_on);
    if (!excludeMessage) {
      await this.channel.sendMessage(`${team.name} took too long to ${stateText}!`);
    }
  ...
  async updateMessage() {
    ...
    let embed = new EmbedBuilder()
      .setTitle(`${this.round.acronym}: (${this.teams[0].name}) vs (${this.teams[1].name})`)
    ...
    if (state <= 2 || (state >= 5 && state <= 7)) {
      embed.addFields({
        name: 'Score',
        value: `
				${emotes.teams[this.teams[0].id]} ${this.teams[0].name} | ${this.teams[0].score} - ${
          this.teams[1].score
        } | ${this.teams[1].name} ${emotes.teams[this.teams[1].id]}`,
      });
    }
    ...
    if (
      ...
      let leaderboard = '';
      let lastMap = (await nodesuClient.beatmaps.getByBeatmapId(this.lastGameData.beatmap_id))[0];
      let { title, artist, version } = lastMap;
      let lastMapId =
        this.mappool.find((map) => map.beatmap_id == lastMap.beatmap_id)?.identifier || 'Warmup';
      ...
      for (const score of this.lastGameData.scores) {
        ...
        // Calculate acc
        let accuracy = 300 * score.count300 + 100 * score.count100 + 50 * score.count50;
        let divisor =
          300 *
    ...
    if (this.state == 1) {
      ...
      let players = this.lobby.slots.filter((x) => x).map((x) => x.user.username);
      for (const team of this.teams) {
        let inLobbyPlayers = team.users.filter((x) => players.includes(x.username));
    ...
    // Beatmap Image
    if (this.beatmap) {
      embed.setImage(`https://assets.ppy.sh/beatmaps/${this.beatmap?.setId}/covers/cover.jpg`);
    }
    ...
    if (bans.length > 0) {
      ...
      for (const ban of bans) {
        ...
        let string =
          teamString[ban.bannedBy.id] == null ? `${ban.identifier}` : `, ${ban.identifier}`;

        teamString[ban.bannedBy.id] =
          teamString[ban.bannedBy.id] == null ? string : (teamString[ban.bannedBy.id] += string);
      }

      banString = `
				${emotes.teams[this.teams[0].id]} **${this.teams[0].name}:** ${teamString[this.teams[0].id] || ''}
				${emotes.teams[this.teams[1].id]} **${this.teams[1].name}:** ${teamString[this.teams[1].id] || ''}
			`;
    ...
    if (this.teams[0].pick_order) {
      ...
      for (const pick of picks) {
        ...
        let string = `${emotes.teams[pick.wonBy?.id] || emotes.loading} **${pick.identifier}**\n`;
    ...
    if (state == -1) {
      ...
      let components = new ActionRowBuilder().addComponents(recoverButton, deleteButton);
      await message.edit({
        content: null,
        embeds: [embed],
    ...
    if (state == 4) {
      ...
      if (this.beatmap == null) {
        embed.setDescription(`${this.teams[this.waiting_on].name} is picking a warmup`);
        embed.setThumbnail(this.teams[this.waiting_on].icon_url);
      } else {
