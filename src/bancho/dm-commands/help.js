  ...
  async exec(msg, options) {
    ...
    if (!msg.channel) {
      ...
      for (let key in commands) {
        ...
        if (command.dm) {
          await msg.user.sendMessage(`• !${command.name} - ${command.desc}`);
        }
      }
    }

    if (msg.channel) {
      msg.user.sendMessage('Here are all available Multiplayer commands: ');
      for (let key in commands) {
        let command = commands[key];
        if (command.mp) {
          await msg.channel.sendMessage(`!${command.name} - ${command.desc}`);
        }
      }
    }
  },
