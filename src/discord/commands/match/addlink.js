...
module.exports = {
  data: new SlashCommandSubcommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('link').setDescription('The link to the lobby').setRequired(true)
    ),
  /**
   *
  ...
  async execute(interaction) {
    ...
    if (!matchId) {
      let embed = new EmbedBuilder()
        .setDescription('**Err**: You are not in a match that requires an MP link.')
        .setColor(Colors.Red);
    ...
    embed
      ...
      .setDescription("<a:loading:970406520124764200> We're currently setting up your match...")
      .setColor('#F88000');
    await interaction.editReply({ embeds: [embed] });

    let match = new MatchManager(matchId, interaction.options.getString('link'));
    try {
      await match.createMatch();
    } catch (e) {
      console.log(e);
      embed = new EmbedBuilder()
        .setDescription('**Err**: We encountered an error while joining the match')
        .setColor(Colors.Red)
        .setFooter({
    ...
    let archiveTimeout = require('../../buttons/start_match')[`match-${match.id}`];
    clearTimeout(archiveTimeout);
    embed
      .setTitle('Match loaded!')
      .setColor(Colors.Green)
      .setDescription('<a:verified:970410957710954636> Check your invite message for more info');
