  ...
  async execute(interaction) {
    ...
    if (!tournament.allow_registrations) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot deregister while registrations are disabled.`)
        .setColor(Colors.Red);
    ...
    if (!team) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot deregister unless you are in a team.`)
        .setColor(Colors.Red);
    ...
    if (matchCheck) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot deregister while your team is match.`)
        .setColor(Colors.Red);
      interaction.editReply({ embeds: [embed] });
      return;
    }
