...
module.exports = {
  data: new SlashCommandSubcommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName('user').setDescription('The user you want to invite').setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (!inviterTeam) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot invite a user if you are not in a team.`)
        .setColor(Colors.Red)
        .setFooter({
    ...
    if (tournament.team_size == 1) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot invite a user if the team size is 1.`)
        .setColor(Colors.Red);
      await interaction.editReply({ embeds: [embed] });
      return;
    }
    ...
    if (duplicateCheck) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot invite a user that is already in your team.`)
        .setColor(Colors.Red);
      await interaction.editReply({ embeds: [embed] });
      return;
    }
    ...
    if (!inviteeData) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: User \`${invitee.tag}\` has not linked their account.`)
        .setColor(Colors.Red);
      await interaction.editReply({ embeds: [embed] });
      return;
    }
    ...
    if (inviteeTeam) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: User \`${inviteeData.username}\` is already in a team.`)
        .setColor(Colors.Red);
      await interaction.editReply({ embeds: [embed] });
      return;
    }
    ...
    if (matchCheck) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot invite while your team is match.`)
        .setColor(Colors.Red);
