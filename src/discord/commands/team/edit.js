  ...
  async execute(interaction) {
    ...
    if (!team) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot edit your team unless you are the owner of the team`)
        .setColor(Colors.Red);
    ...
    if (!tournament.allow_registrations) {
      let embed = new EmbedBuilder()
        .setDescription('**Err**: You cannot edit your team while registration is closed.')
        .setColor(Colors.Red);
    ...
    if (matchCheck) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot make changes to your team while you are in a match`)
        .setColor(Colors.Red);
    ...
    if (tournament.team_size == 1) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: You cannot edit your team if the team size is 1.`)
        .setColor(Colors.Red);
      await interaction.editReply({ embeds: [embed] });
      return;
    }
    ...
    if (options?.name?.length > 25) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: The name of your team cannot be longer than 25 characters.`)
        .setColor(Colors.Red)
        .setFooter({
