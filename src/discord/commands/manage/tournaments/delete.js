...
module.exports = {
  data: new SlashCommandSubcommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('acronym').setDescription('The acronym of the tournament').setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (tournament == null) {
      let embed = new EmbedBuilder()
        .setDescription(`**Err**: No tournament with the acronym \`${acro}\` found.`)
        .setColor(Colors.Red);
    ...
    let embed = new EmbedBuilder().setColor(Colors.Green).setTitle('⚠ WARNING ⚠')
