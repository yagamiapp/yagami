...
module.exports = {
  data: new SlashCommandSubcommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('id_column').setDescription('Column of map IDs').setRequired(true)
    ),
  /**
   *
  ...
  async execute(interaction) {
    ...
    let ids = interaction.options.getString('identifier_column').match(/\S+/g);
    let maps = interaction.options.getString('id_column').match(/\d+/g);
    let acronym = interaction.options.getString('acronym');
    let embed = new EmbedBuilder().setTitle('Importing maps').setColor('#F88000');
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      embed.setDescription(`<a:loading:970406520124764200> Currently Importing: **${identifier}**`);
      await interaction.editReply({ embeds: [embed] });
